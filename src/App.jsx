...
import { ChevronLeft, ChevronRight, MessageCircle, Camera, Tv, Globe, Users, Image as ImageIcon, Activity, X, Crosshair, Zap, Filter, Target, MousePointer2, TrendingUp, Sparkles, AlertCircle, Info } from 'lucide-react';
...
const getStrategicVerdict = (ad, targetCpa) => {
  if (ad.spend > (3 * targetCpa) && ad.conversions === 0) {
    return { title: "CAPITAL BLEED", analysis: "Critical budget leak. Zero conversion intent. Terminate immediately.", colors: "bg-red-700 text-white shadow-xl" };
  }
  if (ad.creativeScore < 3 && ad.lpScore < 5) {
    return { title: "SYSTEMIC FAILURE", analysis: "Total funnel rejection. Both creative and landing page assets are toxic. Rebuild required.", colors: "bg-slate-950 text-white shadow-xl" };
  }
  if (ad.creativeScore < 4) {
    return { title: "CREATIVE REJECTION", analysis: "Algorithm alert. Low quality score is taxing your CPC. Flag for asset replacement.", colors: "bg-orange-600 text-white shadow-xl" };
  }
  if (ad.roas > 3 && ad.cpa <= targetCpa) {
    return { title: "PROVEN WINNER", analysis: "High-yield asset. Performance is optimal. Scale budget aggressively.", colors: "bg-emerald-900 text-white shadow-xl" };
  }
  return { title: "MONITOR", analysis: "Stable baseline. Gathering intelligence for 48h trend analysis.", colors: "bg-indigo-700 text-white shadow-xl" };
};
    ...
    <div className="min-h-screen bg-[#F8F7F2] text-slate-900 p-4 md:p-12 font-['Inter',sans-serif]">
      ...
      <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 border-b border-slate-200 pb-10">
        <div>
          ...
          <p className="text-xl font-medium text-slate-600 flex items-center gap-2">
            <TrendingUp size={20} className="text-emerald-600"/> Growth Intelligence Control Center
          </p>
        </div>
        <div className="bg-white p-8 rounded-[2rem] shadow-2xl border border-slate-100 min-w-[280px]">
          <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-2">Total Budget Leak</p>
          <p className="text-6xl font-black text-red-600 tracking-tighter">${metrics.wasted}</p>
        </div>
      </header>

      {/* STRATEGIC NOTE (HOMEPAGE ONLY) */}
      <div className="mb-10 bg-indigo-50 border border-indigo-100 p-6 rounded-[1.5rem] flex items-start gap-4 shadow-sm">
        <Info className="text-indigo-600 mt-1 shrink-0" size={24} />
        <div>
          <h4 className="font-black text-indigo-900 uppercase text-xs tracking-widest mb-1">Strategic Protocol v2.6</h4>
          <p className="text-indigo-800 text-sm leading-relaxed max-w-4xl">
            In 2026, **Creative is the Targeting**. This dashboard automatically flags any asset with a Creative Score below **4.0** as a "Flagged" risk. Ads spending 3x Target CPA ($150) without conversions are isolated as "Wasted Potential." Monitor the status matrix daily to redistribute capital into "Proven Winners."
          </p>
        </div>
      </div>

      {/* COMMAND MATRIX & PAGINATION */}
      <div className="mb-8 flex flex-wrap justify-between items-center gap-6">
        <div className="flex flex-wrap gap-3 items-center">
            <button onClick={() => setActiveFilter({ type: 'all', value: null })} className={`px-6 py-2.5 rounded-full text-xs font-bold transition-all border ${activeFilter.type === 'all' ? 'bg-slate-900 text-white shadow-xl' : 'bg-white text-slate-600'}`}>All Assets</button>
            <div className="h-6 w-[1px] bg-slate-300 mx-1" />
            {['Kill', 'Scale', 'Flag', 'Monitor'].map(d => (
            <button key={d} onClick={() => setActiveFilter({ type: 'directive', value: d })} className={`px-6 py-2.5 rounded-full text-xs font-bold transition-all border ${activeFilter.value === d ? 'bg-slate-900 text-white shadow-xl' : 'bg-white text-slate-600'}`}>{d.toUpperCase()}</button>
            ))}
        </div>

        {/* PAGINATION CONTROLS */}
        <div className="flex items-center gap-4 bg-white px-6 py-2 rounded-full shadow-md border border-slate-100">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} className="p-1 hover:text-emerald-600 transition-colors"><ChevronLeft size={20}/></button>
            <span className="text-xs font-black uppercase tracking-widest">Page {page}</span>
            <button onClick={() => setPage(p => p + 1)} className="p-1 hover:text-emerald-600 transition-colors"><ChevronRight size={20}/></button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {isLoading ? <tr><td colSpan="3" className="p-40 text-center font-bold text-slate-300 italic text-2xl animate-pulse">Syncing...</td></tr> : 
              ...
              <tr key={i} onClick={() => setSelectedAd(ad)} className={`cursor-pointer transition-all border-l-[8px] border-l-transparent group ${ad.rowClass}`}>
                <td className="p-8">
                  <span className={`px-5 py-2 rounded-full text-[11px] font-black uppercase border-2 ${ad.themeClass}`}>{ad.action}</span>
                </td>
                <td className="p-8">
      ...
      {selectedAd && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-2xl bg-white h-full overflow-y-auto p-12 md:p-16 flex flex-col animate-in slide-in-from-right duration-500 border-l border-slate-100 shadow-2xl">
            
            <button onClick={() => setSelectedAd(null)} className="self-end mb-8 p-3 bg-slate-100 rounded-full text-slate-500 hover:text-slate-900 shadow-sm transition-all hover:scale-110">
              <X size={28} />
            </button>
            ...
            <div className={`p-10 rounded-[2.5rem] shadow-2xl mb-12 flex flex-col gap-6 ${selectedAd.verdict.colors}`}>
              ...
              <p className="text-xl font-medium opacity-90 leading-relaxed">{selectedAd.verdict.analysis}</p>
            </div>

            <div className="grid grid-cols-2 gap-8 mb-16">
              {[
                { label: 'Creative Quality', value: `${selectedAd.creativeScore.toFixed(1)}/10`, color: selectedAd.creativeScore < 4 ? 'text-red-600' : 'text-emerald-600' },
            ...
            <div className="pt-12 border-t-2 border-slate-900 space-y-12 mb-10">
               <h4 className="text-sm font-black uppercase tracking-[0.5em] text-slate-900 pb-4 border-b border-slate-200 flex items-center gap-2">
                 <Sparkles size={18} className="text-indigo-600"/> Asset Data Points
               </h4>
               <div className="grid grid-cols-2 gap-y-12">
                 <div><span className="text-[12px] font-black text-slate-900 uppercase block mb-3">Target Audience</span><span className="text-2xl font-black text-slate-900 uppercase">{selectedAd.target_audience || "Universal"}</span></div>
                 <div><span className="text-[12px] font-black text-slate-900 uppercase block mb-3">Creative Theme</span><span className="text-2xl font-black text-indigo-700 italic">{selectedAd.creative_theme}</span></div>
                 <div><span className="text-[12px] font-black text-slate-900 uppercase block mb-3">Video Completion</span><span className="text-2xl font-black text-slate-900">{selectedAd.vcr > 0 ? `${selectedAd.vcr}%` : 'N/A'}</span></div>
                 <div><span className="text-[12px] font-black text-slate-900 uppercase block mb-3">Efficiency (CPC)</span><span className="text-2xl font-black text-slate-900">${selectedAd.cpc.toFixed(2)}</span></div>
                 <div><span className="text-[12px] font-black text-slate-900 uppercase block mb-3">Ad Frequency</span><span className="text-2xl font-black text-slate-900">{selectedAd.frequency.toFixed(2)}x</span></div>
                 <div><span className="text-[12px] font-black text-slate-900 uppercase block mb-3">Platform Logic</span><span className="text-2xl font-black text-slate-900 uppercase">{selectedAd.identity.name} • {selectedAd.ad_type}</span></div>
               </div>
            </div>
          </div>
        </div>
